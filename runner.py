...
from gyromatman.utils import get_logging, write_results_to_file, compute_metrics, avg_side_metrics, \
...
class Runner(object):
    ...
    def run(self):
        ...
        best_hr1, best_hr3, best_hr10 = -1, -1, -1  
        ...
        for epoch in range(self.args.init_epoch + 1, self.args.epochs + 1):
            ...
            train_loss = self.train_epoch(self.train_loader, epoch) # this calls forward and computes loss with regulalizer
            #train_loss = 0  
            exec_time = time.perf_counter() - start

            if self.is_main_process:
                self.log.info(f'Epoch {epoch} | train loss: {train_loss:.4f} | total time: {exec_time:.2f} secs | {self.args.data} | {self.args.model} | n = {self.args.dims} | p = {self.args.pdim} | k = {self.args.kdim}')
            ...
            if epoch % self.args.val_every == 0:
                start = time.perf_counter()
                metrics, _ = self.evaluate(self.valid_loader)       # this calls forward_eval and computes metrics
                eval_time = time.perf_counter() - start             # based on output scores
                mrr = metrics["MRR"]
                #if self.is_main_process:
                #    for key in ["MRR", "MR", "HR@1", "HR@3", "HR@10"]:
                #        self.writer.add_scalar(f"val/{key}", metrics[key], epoch)

                self.log.info(f"RANK {self.args.local_rank}: Results ep {epoch}: time: {eval_time:.2f} s, "
                              f"tr loss: {train_loss:.1f}, MRR: {mrr * 100:.2f}, MR: {metrics['MR']:.0f}, "
                              f"HR@1: {metrics['HR@1'] * 100:.2f}, "
                              f"HR@3: {metrics['HR@3'] * 100:.2f}, "
                ...
                if metrics['HR@3'] > best_hr3:
                    best_hr3 = metrics['HR@3']
                if metrics['HR@10'] > best_hr10:
                    best_hr10 = metrics['HR@10']

                if self.is_main_process:
                    self.log.critical(f"Best MRR: {best_mrr * 100:.3f}, "
                                  f" Best HR@1: {best_hr1 * 100:.2f}, "
                                  f" Best HR@3: {best_hr3 * 100:.2f}, "
                                  f" Best HR@10: {best_hr10 * 100:.2f}")
    ...
    def train_epoch(self, train_split, epoch_num):
        ...
        for step, triples in enumerate(train_split):
            ...
            # This calls forward that gives -d((M_r (x) H) (+) R, T)^2 + b_h + b_t in Eq. (7)
